...
const HomeScreen = (props) => {
  ...
  const getProducts =  (url) => {
     Axios.get(url)
      .then(r => {setData(r.data.posts)
        getSpecialOffers (30, r.data.posts)
      })
  }
  ...
  const getSpecialOffers =  (discount, offers) => {
    const specialOffers =  offers.filter((data) => data.discount >= discount)
    ...
    return (
      <View style={styles.itemContainer} >
        <View
          style={styles.card}>
          <TouchableOpacity
            onPress={() => props.navigation.navigate('ProductDetails', { product: product })}>
            <Image
          ...
          <TouchableOpacity style={{ backgroundColor: 'white', height: '80%', width: '40%', marginLeft: '5%' }}>
            <View>
              ...
              <View style={{ padding: 10 }}>
                ...
                <Text style={{ color: '#C4C4C4', textDecorationLine: 'line-through', fontSize: 18, width: 180 }}>Rp. {product.price}</Text>
                <Text style={{ color: '#DE884A', fontSize: 20, width: 190 }}>Rp. {product.actualPrice}</Text>
              </View>
            </View>
          </TouchableOpacity>
        </View>
        ...
        <View style={{ backgroundColor: '#F7F6ED', width: '100%', height: '100%' }}>
          ...
          <ImageBackground style={{ backgroundColor: 'white', width: '100%', height: '8%', padding: 5, flexDirection: 'row' }}>
            <View style={{ flex: 1, flexDirection: 'row' }}>
              ...
              <TouchableOpacity style={{ marginTop: 6, marginLeft: '5%' }}>
          ...
          <View>
            <ImageBackground style={{ width: '100%', height: 260, justifyContent: 'center', backgroundColor: '#96A545' }}>
              <View style={{ flexDirection: 'row', width: '100%' }}>
                <View style={{ marginLeft: '2%', padding: 5, marginTop: '7%' }}>
                  ...
                  <TouchableOpacity style={{ padding: 5, width: '100%', borderRadius: 10, backgroundColor: 'white', opacity: 0.5 }}>
                    <Text style={{ alignSelf: 'center', fontSize: 18, color: '#618841', fontWeight: 'bold' }}>DISC UP TO 30%</Text>
                  </TouchableOpacity>
                </View>
                <FlatList
                  horizontal
